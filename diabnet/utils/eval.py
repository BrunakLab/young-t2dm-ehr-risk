...
from diabnet.utils.c_index import concordance_index


def get_probs_golds(test_preds, index=4, return_mask=False):
    """
    ...
    probs_for_eval, golds_for_eval, mask = [], [], []
    for prob_arr, censor_time, gold in zip(
        test_preds["probs"], test_preds["censor_times"], test_preds["golds"]
    ):
    ...
    if return_mask:
        return probs_for_eval, golds_for_eval, mask
    else:
...
def compute_eval_metrics(
    ...
    for index, time in enumerate(args.month_endpoints):
        ...
        if args.eval_auroc:
            ...
            log_statement += " -{}: {} (n={} , c={} )".format(
                key_name, auc, len(golds_for_eval), sum(golds_for_eval)
            )
        ...
        if args.eval_mcc:
            ...
            log_statement += " -{}: {} (n={} , c={} )".format(
                key_name, mcc, len(golds_for_eval), sum(golds_for_eval)
            )
...
def include_exam_and_determine_label(
    followup, censor_time, gold, cumulative_prediction_interval=True
):
    """
...
def compute_mcc(golds_for_eval, probs_for_eval):
    try:
        ...
        tn, fn = n - fp, p - tp
        mcc = (tp * tn - fp * fn) / (
            np.sqrt(((tp + fp) * (fp + tn) * (tn + fn) * (fn + tp))) + 1e-10
        )
    except Exception as e:

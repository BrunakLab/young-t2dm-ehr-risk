...
def ci_bounds(mean, deltas, confidence_interval, num_resamples):
    ...
    index_offset = int((1 - confidence_interval) / 2.0 * num_resamples)
    ...
    while len(deltas) < num_resamples:
        if clusters is None:
            resample_ind = np.random.choice(
                a=range(len(emperical_distribution)), size=num_samples, replace=True
            )
        else:
            resample_clusters = np.random.choice(a=cluster_ids, size=num_clusters, replace=True)
            resample_ind = []
            for cluster in resample_clusters:
                resampled_inds_for_cluster = np.random.choice(
                    a=cluster_to_inds[cluster],

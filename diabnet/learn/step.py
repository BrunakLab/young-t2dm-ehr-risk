...
def model_step(batch, model, train_model, args, fabric):
    """
    ...
    """
    logit = do_forward(model, batch=batch)
    ...
    patient_golds = batch["future_outcome"].data.cpu().numpy()
    exams = batch["exam"]
...
def do_forward(model, batch):
    model_inputs = {k: v for k, v in batch.items() if k in model.inputs}
    logit = model(**model_inputs)
    return logit

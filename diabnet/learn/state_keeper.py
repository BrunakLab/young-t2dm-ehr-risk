...
def get_model_path(save_dir):
    return os.path.join(save_dir, MODEL_PATH)
...
class StateKeeper:
    ...
    def save(self, model, optimizer, epoch, lr, epoch_stats):
        """
        ...
        param_path = os.path.join(self.experiment_dir, PARAM_PATH)
        with open(param_path, "wb") as param_file:
            pickle.dump(param_dict, param_file)

        # Save epoch_stats dict.
        stats_path = os.path.join(self.experiment_dir, STATS_PATH)
        with open(stats_path, "wb") as stats_file:
            pickle.dump(epoch_stats, stats_file)

        # save model
        model_path = os.path.join(self.experiment_dir, MODEL_PATH)

        torch.save(model, model_path)
        # save optimizer
        optimizer_path = os.path.join(self.experiment_dir, OPTIMIZER_PATH)
            ...
            for param_key in optimizer.state_dict()["state"]:
                ...
                for attribute_key in param:
                    ...
                    optimizer.state_dict()["state"][param_key][attribute_key] = (
                        optimizer.state_dict()["state"][param_key][attribute_key].cuda(param_to_device["{}_{}".format(param_key, attribute_key)])
                    )
